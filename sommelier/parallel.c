/*
 * Improved implementations of the matrix algorithms.
 * Several optimizations are added:
 * e.g. multi-threading, loop-unrolling, register reusing, etc
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* macro defines for loop unrolling */
#define LOOP_UNROLLING_4
#define UNROLLING_SIZE 4 /* inner-loop unroll 4 times */
#define J_UNROLLING_SIZE 3 /* outer-level unroll 3 times */

/* cache blocking */
#define BLOCK_SIZE 400 /* the block size in cache blocking */
...
void init_thread_args(struct work_struct *job, int id,
					  const struct smat *a,
					  const struct smat *b,
					  struct smat *r,
					  int start, int end)
...
void smat_mult_t(void *args)
{
	...
	for (jj = 0; jj < size; jj += BLOCK_SIZE)
		for (kk = 0; kk < size; kk += BLOCK_SIZE)
			for (i = s->start_row; i < s->end_row; i++) {
				double *a = adata[i]; /* register reusing */
				int jend = min(jj+BLOCK_SIZE, size);
				int kend = min(kk+BLOCK_SIZE, size);

				int end = kend / UNROLLING_SIZE * UNROLLING_SIZE;
...
void smat_mult(const struct smat *a, const struct smat *b, struct smat *r)
{
	...
	for (i = 0; i < NUM_THREADS; i++) {
		...
		int start = i * nrows;
		int end = (i == NUM_THREADS-1) ? a->rows : start + nrows;
...
void smat_vect_t(void *args)
{
	...
	int size = s->size;
	int end = s->size / UNROLLING_SIZE * UNROLLING_SIZE;
...
void smat_scale(const struct smat *a, const struct smat *alpha, struct smat *r)
{
	...
	int nrows = a->rows / NUM_THREADS; /* number of rows for each thread */
	for (i = 0; i < NUM_THREADS; i++) {
		jobs[i] = malloc(sizeof(struct work_struct));
		if (jobs[i] == NULL) {
			perror(NULL);
